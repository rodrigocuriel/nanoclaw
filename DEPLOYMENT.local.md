...
Codex credential renewal (2026-09-15): Cardi's `Read-only file system (os error 30)` followed OpenAI HTTP 401 `token_revoked`. The Mac login imported into OneCLI on September 11 was being rejected; the reason for revocation was not established. Host/workspace mounts were writable and proxy exclusions remained correct. Codex's refresh attempt encountered the intentionally read-only auth stub.

Using the operator-supplied `/tmp/codex-auth.json` on curielmedia, replaced the existing `Codex` secret through OneCLI's loopback `PATCH /api/secrets/<id>` endpoint, preserving its ID, agent assignments, `openai` type, and `chatgpt.com` host pattern. Authenticated models-endpoint probes through the actual Cardi, Maven, and Amp containers each returned HTTP 200. Restarted those three groups (`restarted: 1` each); `nanoclaw.service` remained active. History was preserved; groups resume on their next message. No conversational probe, failed-message replay, or dormant-agent authentication test was performed. The supplied file was restricted to `0600` and left in place. See [credential renewal procedure](docs/specialist-operations.md#codex-credential-renewal-after-token_revoked-2026-09-15) for diagnosis, safe replacement, and verification details.
